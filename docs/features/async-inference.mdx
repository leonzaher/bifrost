...
See [Durable Tool Approval](/features/tool-approval) for checkpoint and decision details.
...
- The endpoint must also be subscribed to `async_job.completed`/`async_job.failed`/`async_job.awaiting_approval` at the time the event fires. If it isn't, the submit request still succeeds, but no delivery happens.
- Optional `x-bf-async-webhook-context` correlation data is returned inside the signed payload as `data.context`. See [Webhooks](/features/webhooks) for the approval event payload.
