...
Webhooks are the push half of [Async Inference](/features/async-inference). Instead of polling `GET /v1/async/.../{job_id}` until a job reaches a terminal state, register an endpoint and name it on submit, and Bifrost delivers a signed HTTP `POST` when that job pauses for tool approval, completes, or fails.
...
| `async_job.awaiting_approval` | An agent saves a tool approval checkpoint. |
...
- The endpoint must still be enabled and subscribed to the resulting event (`async_job.completed`, `async_job.failed`, or `async_job.awaiting_approval`) when it fires. If it isn't, the job proceeds normally but no delivery is enqueued.
- Jobs submitted without the header never trigger a webhook, regardless of how many endpoints are registered.

Use `x-bf-async-webhook-context` to attach up to 4096 bytes of opaque correlation data. It is stored with the job and returned as `data.context` inside the signed payload. Do not put credentials in it.
...
| Field | Meaning |
|---|---|
| `event` | `async_job.completed`, `async_job.failed`, or `async_job.awaiting_approval`. |
| `data.job_id` | The async job id. |
| `data.status` | `completed`, `failed`, or `awaiting_approval`. |
| `data.context` | Opaque correlation data supplied at submission. |
| `data.tool_call_request_id` | Saved approval checkpoint ID for an approval event. |
| `data.calls` | Pending tool calls for an approval event, each with `id` and `function.name` / `function.arguments` (a JSON string). Omitted if the batch was already decided when the event was delivered. |
| `data.justification` | Approval-batch explanation extracted from each call's `_injected_justification` argument. Missing, blank, or non-string values use `No justification provided`. Multiple calls produce newline-separated explanations prefixed with their tool names. |
...
Approval events include the pending-call batch regardless of `include_response` and never inline a response. Submit decisions as described in [Async Inference](/features/async-inference#durable-tool-approval).
...
3. Choose the **events** to subscribe to (`async_job.completed`, `async_job.failed`, `async_job.awaiting_approval`).
...
| `events` | array | Yes | Subscribed events: `async_job.completed`, `async_job.failed`, `async_job.awaiting_approval`. |
